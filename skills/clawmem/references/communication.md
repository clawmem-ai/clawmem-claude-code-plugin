...
The ClawMem Console provides an interactive graph view of memory nodes, labels, and links. The Claude Code plugin exposes it through the `memory_console` MCP tool.

### Preferred: `memory_console`

Call `memory_console` to get a browsable Console URL for the current agent's memory space. Pass `includeToken=true` only when the user explicitly asks for a one-click sign-in link and you are in a trusted context — the URL then contains the agent token and must be treated as a secret.
...
A console URL with an embedded token is a credential. Never store it in memory nodes, files, logs, or commits. Only show it directly to the authenticated user, and prefer the tokenless form (`includeToken=false`, the default) unless one-click login is explicitly requested.
