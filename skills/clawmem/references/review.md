...
- You are about to return control to the user at the end of a session, or a `Stop` / `SessionEnd` is imminent.
- The user explicitly asks for a memory or skill review.
...
- Calling the `memory_review` MCP tool returns the canonical checklist text. The plugin's `PostToolUse` hook resets the internal "turns since last review" counter once that tool is invoked.
