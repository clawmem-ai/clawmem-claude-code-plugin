...
ClawMem is routed per agent identity, not through one global repo or token. State lives in `$CLAUDE_PLUGIN_DATA/state.json` (the plugin sets this; default is `~/.claude/plugins/data/clawmem-claude-code-plugin@clawmem/` for marketplace installs, `.data-dev/` for local dev).
...
- `CLAWMEM_REPO` is the repo chosen for the current operation (optional first arg to the exports script overrides this)
- If `CLAWMEM_TOKEN` is empty, this agent identity has not been provisioned yet — start a fresh Claude Code session so bootstrap runs
...
test -n "$CLAWMEM_REPO" || { echo "ClawMem repo missing for login $CLAWMEM_LOGIN"; exit 1; }
test -n "$CLAWMEM_TOKEN" || { echo "ClawMem token missing for login $CLAWMEM_LOGIN"; exit 1; }
...
| Plugin state looks stale | Inspect `$CLAUDE_PLUGIN_DATA/state.json` directly; remove it and restart Claude Code to force bootstrap |
| Conversation mirror returns `404` | The cached conversation issue was deleted; the plugin recreates it on the next session |
| New session gets `401 Unauthorized` | Re-read the current agent route. If this is first use, start a fresh session so the plugin can finish provisioning |
| Agent uses the wrong memory repo | Resolve the `route.defaultRepo` field in plugin state; pass an explicit `repo` to memory tools when targeting a non-default space |
...
- any `collaboration_*` write tool — all of those require `confirmed=true` after a preview
- plugin state file deletions or edits
